...
decoder_start(struct decode *dec)
{
	...
	while (1) {
		...
		if ((dec->cmdl->dst_scheme == PATH_V4L2) || (dec->cmdl->dst_scheme == PATH_IPU) || (dec->cmdl->dst_scheme == PATH_G2D)) {
			if (deblock_en) {
				if (dec->cmdl->dst_scheme == PATH_G2D)
					deblock_fb->bufY = disp->g2d_buffers[disp->buf.index]->buf_paddr;
				else
					deblock_fb->bufY = disp->buffers[disp->buf.index]->offset;
			...
			if (!cpu_is_mx27())
				if (dec->cmdl->dst_scheme == PATH_V4L2)
					...
					err = g2d_put_data(dec, actual_display_index, dec->decoded_field[actual_display_index], dec->cmdl->fps);
				else
	...
	if ((dst_scheme == PATH_V4L2) || (dst_scheme == PATH_IPU) || (dst_scheme == PATH_G2D)) {
		...
		if (rotation.rot_en) {
			...
			if (dst_scheme == PATH_V4L2)
				...
				disp = g2d_display_open(dec, totalfb, rotation, rotCrop);
			else
			...
			if (dst_scheme == PATH_V4L2)
				...
				disp = g2d_display_open(dec, totalfb, rotation, dec->picCropRect);
			else
			...
			for (i = 0; i < totalfb; i++) {
                ...
                if (dec->cmdl->mapType == LINEAR_FRAME_MAP) {
                    if (dst_scheme == PATH_V4L2)
                        ...
                        fb[i].bufY = disp->g2d_buffers[i]->buf_paddr;
                    else
