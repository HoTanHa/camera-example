...
/*---------------------------------------------------------------------------*/
/***************************************************************************/
// original file : https://gist.github.com/mike168m/6dd4eb42b2ec906e064d
// patch by abureau le 15/10/19
#include <iostream>
#include <stdio.h>
#include <stdlib.h>
#include <linux/ioctl.h>
...
//   imageFormat.fmt.pix.field = V4L2_FIELD_ANY;
	imageFormat.fmt.pix.field = V4L2_FIELD_INTERLACED_TB;
//    imageFormat.fmt.pix.field = V4L2_FIELD_ALTERNATE;
    // tell the device you are using this format
    if(ioctl(fd, VIDIOC_S_FMT, &imageFormat) < 0){
        perror("Device could not set format, VIDIOC_S_FMT");
        return 1;
...
//************************************************************/    
	...
	char pic_name[50];
	sprintf(pic_name, "pic_%d%02d%02d_%02d%02d%02d.jpeg", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
    outFile.open(pic_name, ios::binary| ios::app);
 //**********************************************************************/
    int bufPos = 0, outFileMemBlockSize = 0;  // the position in the buffer and the amoun to copy from
                                        // the buffer
    int remainingBufferSize = bufferinfo.bytesused; // the remaining buffer size, is decremented by
